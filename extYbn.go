...
func parseYbn(oriStm []byte, key []byte) (script ybnInfo, err error) {
	...
	if !bytes.Equal(header.Magic[:], []byte("YSTB")) ||
	...
	if !bytes.Equal(key, []byte("\x00\x00\x00\x00")) {
	...
	//err1 := os.WriteFile(strings.Join(s, ""), oriStm, 0644)
	//fmt.Println(err1)
...
func decodeScriptString(script *ybnInfo, ops *keyOps, enc encoding.Encoding) {
	decode := DecoderMix(enc)
	for i := range script.Insts {
		inst := &script.Insts[i]
		for j := range inst.Args {
			res := &inst.Args[j].Res
			if len(res.Res) != 0 {
				res.ResStr = decode(res.Res)
			} else if inst.Op == ops.msgOp {
				res.ResStr = decode(res.ResRaw)
			}
		}
	}
}
...
func extTxtFromYbn(script *ybnInfo, ops *keyOps, enc encoding.Encoding) (txt []string, err error) {
	decode := DecoderMix(enc)
		...
		if inst.Op == ops.msgOp {
			...
			if isFunctionToExtract(inst.Args[0].Res.Res) {
				for _, arg := range inst.Args[1:] {
					if arg.Type == 3 &&
						!bytes.Equal(arg.Res.Res, []byte(`""`)) &&
						!bytes.Equal(arg.Res.Res, []byte(`''`)) {
						txt = append(txt, decode(arg.Res.Res))
					}
				}
			}
...
func parseYbnFile(ybnName, outScriptName, outTxtName string, key []byte, ops *keyOps, enc encoding.Encoding) bool {
	...
	if outScriptName != "" {
		...
		if gVerbose {
			...
			decodeScriptString(&script, ops, enc)
		}
		...
		os.WriteFile(outScriptName, out, os.ModePerm)
	}
	if outTxtName != "" {
		logln("extracting text from script...")
		txt, err := extTxtFromYbn(&script, ops, enc)
		if err != nil {
			fmt.Println("error when extracting txt:", err)
			return false
		}
		logln("encoding text and writing...")
		lines := strings.Join(txt, "\r\n")
		out := Encoder(unicode.UTF8)(lines)
...
func packLineToResource(arg argInfo, ns []byte) []byte {
	if arg.Type == 3 {
		var bf bytes.Buffer
		var resInfo YResInfo
...
func packTxtToYbn(script *ybnInfo, stm []byte, txt []string, ops *keyOps, enc encoding.Encoding, key []byte) (newStm []byte, err error) {
	...
	argIdx := 0
	txtIdx := 0
	encode := EncoderMix(enc)
	for _, inst := range script.Insts {
		if inst.Op == ops.msgOp {
			ns := packLineToResource(inst.Args[0], encode(txt[txtIdx]))
			txtIdx++
			resTail.Write(ns)
			...
			if isFunctionToExtract(inst.Args[0].Res.Res) {
				for i, arg := range inst.Args[1:] {
					if arg.Type == 3 &&
						!bytes.Equal(arg.Res.Res, []byte(`""`)) &&
						!bytes.Equal(arg.Res.Res, []byte(`''`)) {
						ns := packLineToResource(arg, encode(txt[txtIdx]))
						txtIdx++
						resTail.Write(ns)
...
func packYbnFile(ybnName, txtName, outYbnName string, key []byte, ops *keyOps, enc encoding.Encoding) bool {
	logln("reading file:", ybnName)
	oriStm, err := os.ReadFile(ybnName)
	if err != nil {
		fmt.Println(err)
	...
	src, err := os.ReadFile(txtName)
	if err != nil {
		fmt.Println(err)
		return false
	}
	txt := Decoder(unicode.UTF8)(src)
	lines := strings.Split(txt, "\r\n")
	logf("reading text finished, %d lines\n", len(lines))
	logln("packing text to ybn...")
	newStm, err := packTxtToYbn(&script, oriStm, lines, ops, enc, key)
	if err != nil {
		fmt.Println(err)
		return false
	}
...
func main() {
	...
	codePage := flag.String("cp", "SHJIS", "specify code page")
	...
	var ret bool
	if *isExtract {
		ret = parseYbnFile(*inYbnName, *jsonName, *txtName, key[:], &ops, enc)
	} else {
		ret = packYbnFile(*inYbnName, *txtName, *outYbnName, key[:], &ops, enc)
	}
	if !ret {
		retCode = 1
	}
